...
function calculation() {
  ...
  if (volFlowrate.textContent >= 170 & volFlowrate.textContent <= 850){
    polytropic.textContent = 0.63
  }
  if (volFlowrate.textContent > 850 & volFlowrate.textContent <= 12700){
    polytropic.textContent = 0.74
  }
  if (volFlowrate.textContent > 12700 & volFlowrate.textContent <= 340000){
  ...
  if(polytropic.textContent != '-' & adiabat.value != 1 & pInput.value != pOutput.value) {
    if (Number(tOutput.value) < Number(gasTemperature.textContent)) {
      ...
      checkT.style.display = 'block';
      checkT.className = 'card text-white bg-danger d-print-none';
      checkT.innerHTML = '<div class="card-body"><h5 class="card-title">Температурное условие не выполняется!</h5></div>'
    }
  }
}
